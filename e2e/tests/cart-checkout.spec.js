...
test.describe('Cart and checkout regression coverage', () => {
  test('authenticated shopper can remove a seeded storefront product from the cart', async ({ page }) => {
    const shell = new AppShellPage(page);
    const authPage = new AuthPage(page);
    const storefront = new StorefrontPage(page);
    const cartPage = new CartPage(page);
    const user = buildUser('remove');
    ...
    await cartPage.removeProduct(firstTitle);
