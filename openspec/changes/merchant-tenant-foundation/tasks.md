...
- [x] 0.1 Confirm `merchant-auth-foundation` is complete and define how authenticated user identity maps into the tenant-context contract.

## 1. Merchant Onboarding Contract

- [x] 1.1 Define the canonical merchant signup route at `/{tenantSlug}/signup`, minimum onboarding fields, slug validation rules, and first-slice tenant creation contract.
- [x] 1.2 Decide whether the first merchant-admin user is created in the same flow or as an immediate follow-up and document the resulting contract.
- [x] 1.3 Define the onboarding success-message lifecycle so tenant-created confirmation is shown only for the current successful submission and clears on route exit, reload, and later return.
...
- [x] 2.2 Add canonical path-based tenant routing for `/`, `/{tenantSlug}/`, and `/{tenantSlug}/signup` in `react-ui` and `api-gateway-microservice`.
